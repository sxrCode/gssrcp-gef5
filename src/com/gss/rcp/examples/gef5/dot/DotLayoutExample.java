...
public class DotLayoutExample extends AbstractZestExample {
	...
	public class DotNativeLayout implements ILayoutAlgorithm {
		...
		public void applyLayout(LayoutContext layoutContext, boolean clean) {
			...
			Graph dotGraph = new GraphCopier(new IAttributeCopier() {
				...
				public void copy(IAttributeStore source, IAttributeStore target) {
					if (source instanceof Node && target instanceof Node) {
						...
						DotAttributes._setName((Node) target, (String) ((Node) source).attributesProperty().get(LABEL));
					}
				}
			...
			File tmpFile = DotFileUtils.write(new DotExport().exportDot(dotGraph));
	...
	protected Graph createGraph() {
		...
		return new Graph.Builder().attr(ZestProperties.LAYOUT_ALGORITHM__G, new DotNativeLayout()).node(paper)
				.attr(LABEL, paper) //
