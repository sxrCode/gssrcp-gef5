...
public class ConnectionSnippet extends AbstractFxExample {
	...
	public Scene createScene() {
		GeometryNode<RoundedRectangle> end1 = new GeometryNode<>(new RoundedRectangle(50, 50, 30, 30, 20, 20));
		...
		// use a shape
		javafx.scene.shape.Rectangle end2 = new javafx.scene.shape.Rectangle(200, 50, 30, 30);
		end2.setArcWidth(20);
		...
		connection2.setStartAnchor(new DynamicAnchor(start, new OrthogonalProjectionStrategy()));
		connection2.setEndAnchor(new DynamicAnchor(end2, new OrthogonalProjectionStrategy()));
		...
		EventHandler<MouseEvent> dragNodeHandler = new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent event) {
				if (MouseEvent.MOUSE_PRESSED.equals(event.getEventType())) {
					...
					initialMouse = new Point2D(event.getSceneX(), event.getSceneY());
					initialTranslate = new Point2D(node.getTranslateX(), node.getTranslateY());
				} else {
					// move the node by the mouse offset
					node.setTranslateX(initialTranslate.getX() + event.getSceneX() - initialMouse.getX());
					node.setTranslateY(initialTranslate.getY() + event.getSceneY() - initialMouse.getY());
				}
			}
		};
