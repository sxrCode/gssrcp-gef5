...
public class DynamicAnchorShapeSnippet extends AbstractFxExample {
	...
	public Scene createScene() {
		...
		shapeChoiceBox.getItems().addAll("Arc", "Circle", "CubicCurve", "Ellipse", "Line", "Path", "Polygon",
				"Polyline", "QuadCurve", "Rectangle", "Star Polygon");
		...
		refPoint.boundsInParentProperty().addListener(new ChangeListener<Bounds>() {
			@Override
			public void changed(javafx.beans.value.ObservableValue<? extends Bounds> observable, Bounds oldValue,
					Bounds newValue) {
				Point2D refInScene = refPoint.localToScene(refPoint.getCenterX(), refPoint.getCenterY());
				Point2D refInLocal = anchorLine.sceneToLocal(refInScene);
				if (anchor != null) {
					anchor.getComputationParameter(anchorKey, AnchoredReferencePoint.class)
		...
		refPoint.setOnMouseDragged(new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent event) {
				Point2D positionInParent = refPoint.getParent().sceneToLocal(event.getSceneX(), event.getSceneY());
				refPoint.relocate(positionInParent.getX(), positionInParent.getY());
			}
		});

		// register for choice changes
		shapeChoiceBox.valueProperty().addListener(new ChangeListener<String>() {
			@Override
	...
	protected void showShape(Group selectedShapeGroup, String item) {
		// unregister anchor
		if (anchor != null) {
			anchor.positionsUnmodifiableProperty().removeListener(anchorPositionChangeListener);
		...
		if ("Arc".equals(item)) {
			...
			shape = new Path(new MoveTo(10, 10), new LineTo(80, 10), new QuadCurveTo(50, 50, 80, 80),
			...
			shape = new Polygon(50, 10, 60, 40, 90, 50, 60, 60, 50, 90, 40, 60, 10, 50, 40, 40);
		} else {
			throw new IllegalStateException("Unsupported selection: <" + item + ">.");
		}
