...
public class MouseDragSnippet extends AbstractFxExample {
	...
	private ObservableSet<Node> nodesUnderMouse = FXCollections.observableSet(new HashSet<Node>());
	...
	public Scene createScene() {
		...
		stackPane.getChildren().addAll(contentLayer, handleLayer, feedbackLayer);
	...
	private Node generate(double w, double h) {
		...
		rectangle.setFill(new Color(Math.random(), Math.random(), Math.random(), 0.5));
	...
	protected void onMousePress(MouseEvent event) {
		...
		startLayoutPosition = new Point2D(pressed.getLayoutX(), pressed.getLayoutY());
	...
	protected void onSceneSizeChange(double width, double height) {
		...
		for (int i = 0; i < count; i++) {
			Node n = handleLayer.getChildren().get((int) (Math.random() * count / 2));
			Node m = null;
			while (m == null || m == n) {
				m = handleLayer.getChildren().get((int) (Math.random() * count / 2));
			}
