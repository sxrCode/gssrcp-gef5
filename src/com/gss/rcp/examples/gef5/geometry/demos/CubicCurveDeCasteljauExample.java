...
public class CubicCurveDeCasteljauExample extends AbstractExample {
	...
	protected ControllableShape[] getControllableShapes() {
		return new ControllableShape[] { new ControllableShape() {
			{
				/*
				 * These are the control points used to construct the CubicCurve later.
				 */
				addControlPoints(new Point(100, 200), new Point(200, 100), new Point(300, 300), new Point(400, 200));
			}
			...
			public void onDraw(GC gc) {
				/*
				 * Draws the CubicCurve and the de Casteljau construction for the current
				 * parameter value.
				 */
				...
				/*
				 * Retrieve control points to compute the linear interpolations of the de
				 * Casteljau algorithm.
				 */
				Point[] points = getPoints();

				/*
				 * Define the colors for the intermediate lines. We have three stages and
				 * therefore three different colors for a cubic Bezier curve. This is the case,
				 * because the de Casteljau algorithm reduces the number of control points in
				 * each iteration until it reaches the actual point on the curve.
				 */
				int[] colors = new int[] { SWT.COLOR_DARK_GREEN, SWT.COLOR_BLUE, SWT.COLOR_DARK_RED };

				for (int ci = 0; ci < colors.length; ci++) {
					for (int i = 0; i < 3 - ci; i++) {
						// set line color
						gc.setForeground(Display.getCurrent().getSystemColor(colors[ci]));

						// draw line
						gc.drawLine((int) points[i].x, (int) points[i].y, (int) points[i + 1].x, (int) points[i + 1].y);
						...
						// draw point
						gc.drawOval((int) (points[i].x - 2), (int) (points[i].y - 2), 4, 4);
					}
				}
			}
		} };
	...
	public void onInit() {
		/*
		 * Creates the slider to be able to change the parameter value for which the de
		 * Casteljau algorithm is demonstrated. (Just SWT buzz.)
		 */
